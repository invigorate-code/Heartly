...
export class CreateClientTable1754250023000 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        // Create indexes
        await queryRunner.query(`CREATE INDEX "IDX_client_tenant_id" ON "client" ("tenantId")`);
        await queryRunner.query(`CREATE INDEX "IDX_client_facility_id" ON "client" ("facilityId")`);
        await queryRunner.query(`CREATE INDEX "IDX_client_deleted_at" ON "client" ("deleted_at")`);
        await queryRunner.query(`CREATE INDEX "IDX_client_is_deleted" ON "client" ("isDeleted")`);
        await queryRunner.query(`CREATE INDEX "IDX_client_uci" ON "client" ("uci")`);
        await queryRunner.query(`CREATE INDEX "IDX_client_birth_date" ON "client" ("birthDate")`);
        await queryRunner.query(`CREATE INDEX "IDX_client_tenant_id_deleted_at" ON "client" ("tenantId", "deleted_at")`);
        await queryRunner.query(`CREATE INDEX "IDX_client_facility_id_is_deleted" ON "client" ("facilityId", "isDeleted")`);
        await queryRunner.query(`CREATE INDEX "IDX_client_tenant_id_facility_id" ON "client" ("tenantId", "facilityId")`);

        // Unique constraint for UCI within tenant (only for non-deleted records)
        await queryRunner.query(`CREATE UNIQUE INDEX "IDX_client_tenant_id_uci_unique" ON "client" ("tenantId", "uci") WHERE "deleted_at" IS NULL`);
