...
export class CreateUserActionAuditLogTable1754250024000
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE INDEX "IDX_audit_log_details_gin" ON "user_action_audit_log" USING gin(details)`,
    );

    // Partial index for non-deleted audit logs
    await queryRunner.query(
      `CREATE INDEX "IDX_audit_log_active_logs" ON "user_action_audit_log" ("targetTenantId", "created_at") WHERE "deleted_at" IS NULL`,
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DROP INDEX "IDX_audit_log_tenant_action_created_at"`,
    );
